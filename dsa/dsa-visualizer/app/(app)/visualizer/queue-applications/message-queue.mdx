# MESSAGE QUEUE – COMPLETE EXPLANATION

## 🔹 What is a Message Queue?

A Message Queue (MQ) is a communication mechanism that allows different programs or services to:

- Send messages
- Store them temporarily
- Deliver them asynchronously to receivers

It follows the FIFO (First In First Out) principle.

## 🔹 Why Message Queues?

In real systems, services should not wait for each other.
...
2. Message is placed in the queue
3. Consumer reads message
4. After processing, message is removed
...
- Each message consumed only once
...
"A Message Queue is an asynchronous communication system where producers send messages to a queue and consumers process them later, enabling decoupled and scalable architectures."
...
```
Order Service → [Queue] → Restaurant Service
```

Even if the restaurant service is slow, the order is safely stored in the queue.# Message Queue System
